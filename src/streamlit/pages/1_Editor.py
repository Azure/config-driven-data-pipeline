...
general_view, stg_view, std_view, srv_view, vis_view, code_view, preview_view = st.tabs(["General", "Staging", "Standardization", "Serving", "Visualization", "JSON View", "Preview"])
...
with stg_view:
    ...
    for i in range(len(current_pipeline_obj["staging"]) ):
        ...
        target_name = current_pipeline_obj['staging'][i]['output']['target']
        stg_name = st.text_input(f'Dataset Name', key=f"stg_{i}_name", value=target_name)
        if stg_name:
            current_pipeline_obj['staging'][i]['output']['target'] = stg_name
            current_pipeline_obj['staging'][i]['name'] = stg_name
            ...
            with st.expander(stg_name+" Settings"):
                ...
                if 'description' not in current_pipeline_obj['staging'][i]:
                    current_pipeline_obj['staging'][i]['description'] = ""
                stg_desc = st.text_area(f'Dataset Description', key=f"stg_{i}desc", value=current_pipeline_obj['staging'][i]['description'])
                if stg_desc:
                    current_pipeline_obj['staging'][i]['description'] = stg_desc
                ...
                if 'sampleData' in current_pipeline_obj['staging'][i] and len(current_pipeline_obj['staging'][i]['sampleData']) > 0:
                    sampleData = current_pipeline_obj['staging'][i]['sampleData']
        ...
        if i != len(current_pipeline_obj["staging"]) - 1:
            st.divider()

    if len(current_pipeline_obj["staging"]) == 0:
    ...
    for i in range(len(current_pipeline_obj["standard"])):
        ...
        if std_name:
            ...
            with st.expander(std_name+" Settings"):
                ...
                std_desc = st.text_area(f'Transformation Description', key=f'std_{i}_description', value=current_pipeline_obj['standard'][i]['description'])
                if std_desc:
                    current_pipeline_obj['standard'][i]['description'] = std_desc
    ...
    for i in range(len(current_pipeline_obj["serving"])):
        ...
        if srv_name:
            ...
            with st.expander(srv_name+" Settings"):
                ...
                srv_desc = st.text_area(f'Aggregation Description', key=f'srv_{i}_description', value=current_pipeline_obj['serving'][i]['description'])
                if srv_desc:
                    current_pipeline_obj['serving'][i]['description'] = srv_desc
...
# st.subheader('Chart')
    ...
    for i in range(len(current_pipeline_obj["visualization"])):
        target_name = current_pipeline_obj['visualization'][i]['name']
        ...
        if vis_name:
            ...
            with st.expander(vis_name+" Settings"):
                current_pipeline_obj['visualization'][i]['name'] = vis_name

                if 'description' not in current_pipeline_obj['visualization'][i]:
                    current_pipeline_obj['visualization'][i]['description'] = ""

                vis_desc = st.text_area(f'Chart Description', key=f'vis_{i}_description', value=current_pipeline_obj['visualization'][i]['description'])
                if vis_desc:
                    current_pipeline_obj['visualization'][i]['description'] = vis_desc

                serving_datasets = []
                for serving_tasks in current_pipeline_obj["serving"]:
                ...
                if vis_dataset:
                    current_pipeline_obj['visualization'][i]['input'] = vis_dataset
                    ...
                    if f'vis_{vis_name}_data' in st.session_state:
                        ...
                        for j in range(len(chart_types)):
                            if chart_types[j] == current_pipeline_obj['visualization'][i]['type']:
                        ...
                        if chart_type:
                            current_pipeline_obj['visualization'][i]['type'] = chart_type
                            cols = chart_data.columns.values.tolist()
                            if chart_type == 'Bar Chart' or chart_type == 'Line Chart' or chart_type == 'Area Chart' or chart_type == 'Scatter Chart':
                                ...
                                y_axis = st.selectbox('Y Axis', cols, key=f'vis_y_axis_{i}', index=selected_y_axis_index)
                                if x_axis:
                                    current_pipeline_obj['visualization'][i]['x_axis'] = x_axis
                                if y_axis:
                                    current_pipeline_obj['visualization'][i]['y_axis'] = y_axis
                                    
                                if chart_type == 'Scatter Chart':
                                    selected_scatter_size_index = 2 if len(cols) > 2 else 0
                                    for j in range(len(cols)):
                                        if cols[j] == current_pipeline_obj['visualization'][i]['scatter_size']:
                                            selected_scatter_size_index = j
                                    scatter_size = st.selectbox('Size', cols, key=f'vis_size_{i}', index=selected_scatter_size_index)
                                    if scatter_size:
                                        current_pipeline_obj['visualization'][i]['scatter_size'] = scatter_size

                            elif chart_type == 'Pie Chart':
                                selected_cate_axis_index = 0
                                selected_val_axis_index = 1 if len(cols) > 1 else 0
                                for j in range(len(cols)):
                                    if cols[j] == current_pipeline_obj['visualization'][i]['cate_axis']:
                                        selected_cate_axis_index = j
                                    if cols[j] == current_pipeline_obj['visualization'][i]['val_axis']:
                                        selected_val_axis_index = j
                                cate_axis = st.selectbox('Category', cols, key=f'vis_cate_axis_{i}', index=selected_cate_axis_index)
                                if cate_axis:
                                    current_pipeline_obj['visualization'][i]['cate_axis'] = cate_axis
                                val_axis = st.selectbox('Value', cols,key=f'vis_val_axis_{i}', index=selected_val_axis_index)
                                if val_axis:
                                    current_pipeline_obj['visualization'][i]['val_axis'] = val_axis
        ...
        if i != len(current_pipeline_obj["visualization"]) - 1:
            st.divider()

    if len(current_pipeline_obj["visualization"]) == 0:
...
with code_view:
    ...
    # st.write(str(current_pipeline_obj))
