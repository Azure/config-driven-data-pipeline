...
st.markdown("# AI Assiatant")
st.sidebar.header("AI Assiatant")
...
st.subheader('Generate tables and sample data')
pipeline_name = st.text_input('Pipeline name',
                              key='pipeline_name_ai_assistance',
                              value=current_pipeline_obj.get("name", ""),
                              disabled=True)
pipeline_industry = st.text_input('Industry',
                                  key='pipeline_industry_ai_assistance',
                                  value=current_pipeline_obj.get("industry", ""),
                                  disabled=True)
pipeline_desc = st.text_area('Pipeline description',
                             key='pipeline_description_ai_assistance',
                             value=current_pipeline_obj.get("description", ""),
                             disabled=True)
...
try:
    ...
    for table in tables:
        ...
        with st.expander(table["table_name"]):
            ...
            st.subheader(f"Generate sample data by AI assistant")
            ...
            if f"{gen_table_name}_smaple_data_generated" not in st.session_state:
                ...
                if not st.session_state[f"{gen_table_name}_smaple_data_generated"]:
                    with generate_sample_data_col2:
                        with st.spinner('Generating...'):
                            sample_data = openai_api.generate_sample_data_mock(pipeline_industry, 
                                                                            rows_count,
                                                                            table,
                                                                            data_requirements)
